...
- **`FORK-ONLY`:** B-001 (git stage pathspec), B-002 (keyboard resize), B-005/B-006/B-007/B-008 (mono palettes, mono fill-wipe, telemetry data defects, sidebar headings — all consequences of fork-only features, so they cannot exist upstream).
