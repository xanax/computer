...
- **Switching chat tabs** no longer refetches. Hidden panels skip `loadChat` on mount; a click reloads only if that chat streamed, compacted, or missed a reconnect while it was hidden. Socket reconnect no longer refetches every mounted chat. Off-screen tab wrappers use `content-visibility: hidden` so the browser can skip laying them out.
- **Concluded chats deep-sleep.** Closing a chat drops its tab and unmounts the panel; it is not deleted, and reopening (sidebar/history) creates a fresh tab. Load also strips leftover tabs for chats that are already closed.
